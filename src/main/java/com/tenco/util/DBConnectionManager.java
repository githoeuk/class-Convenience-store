...
// DB에 접근해서 Connection 객체를 생성하는 역할의 클래스이다.
...
public class DBConnectionManager {
    ...
    // 1. static : 클래스 레벨에서 하나만 유지하는 커넥션 풀(hikari)
    private static final HikariDataSource dataSource;

    // static 초기화 블럭 : 클래스가 메모리에 로딩될 때 단 1번만 실행하는 녀석
    static {
        ...
        config.setIdleTimeout(600000);  // 유후 커넥션 유지 시간
    ...
    // 외부에서 접근할 방법이 있어야 함
    // 사용자가 호출하는 매서드 생성 : 풀에서 커넥션 객체 하나를 빌려 온다.
    public static Connection getConnection() throws SQLException {
        return dataSource.getConnection();
    } // end of getConnection

    // 애플리케이션 종료 시 커넥션 풀을 완전히 닫는다.(메모리 누수 방지)
